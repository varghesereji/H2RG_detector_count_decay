...
def Fitting(channel):
    ...
    for flux_bin in range(500,60000,interval):
        ...
        if not np.isnan(avg_count) and delta_change_cube[-1]<0:
            ...
            try:
                fitting_data_variables, error = curve_fit(objective,time1,delta_change_cube,np.array([0.5,0.5,0.0]),sigma = Sigma)

                c,d,a = fitting_data_variables
                print(error)
                dc1 = np.sqrt(error[0][0])
                dd = np.sqrt(error[1][1])
                da = np.sqrt(error[2][2])
                plt.figure(figsize=(16,9))
                plt.plot(time1,delta_change_cube,'o-',color='blue')
                plt.plot(time1,objective(time1,c,d,a),'--',color='green')
                plt.xlabel('time')
                plt.ylabel('count')
