...
def objective1(x,t,C,S,lamb):
    Objective = np.append((x[0]*(np.exp(-x[1]*t)-1)-C)/S,np.sqrt(lamb*np.abs(x[0])))
    Objective = np.append(Objective,np.sqrt(lamb*np.abs(x[1])))
...
def Fitting(lamb,channel):
    ...
    for flux_bin in range(500,60000,interval):
        ...
        if not np.isnan(avg_count) and delta_change_cube[-1]<0:
            ...
            try:
                ...
                fitting_data_variables= least_squares(objective1,x0=np.array([1.0,0.5]),args=(time1,delta_change_cube,Sigma,lamb))
                error = calculate_cov(fitting_data_variables)
                a,b = fitting_data_variables.x
    ...
    return avs,bvs,er_a,er_b,counts

def saving_files(lamb,channel):
    avs,bvs,er_a,er_b,counts = Fitting(lamb,channel)

    np.save('a_values/avs_{0}_{1}.npy'.format(channel,lamb),np.array(avs))
    np.save('b_values/bvs_{0}_{1}.npy'.format(channel,lamb),np.array(bvs))
    np.save('Count_values/avs_{0}_{1}.npy'.format(channel,lamb),np.array(counts))
    np.save('era_values/eravs_{0}_{1}.npy'.format(channel,lamb),np.array(er_a))
    np.save('erb_values/erbvs_{0}_{1}.npy'.format(channel,lamb),np.array(er_b))
