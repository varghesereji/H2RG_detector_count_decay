...
for q in range(4):
    ...
    Sigma2 = np.load('/home/varghese/Desktop/DP1/Codes_Dataset_3/9_readout_error/Long_exp_Variance_channel_{}.npy'.format(q))
    ...
    for flux_bin in range(500,60000,count_range):
        ...
        delta_change_cube = np.nanmedian((dc_0G[10:,pix_mask]-dc_0G[10,pix_mask])/dc_0G[10,pix_mask],axis=1)
        ...
        if np.sum(pix_mask) > 0:
            
            Sigma = Sigma1/(flux_bin*np.sqrt(np.sum(pix_mask)))
            ...
            try:
                ...
                slope_1 = d_0G / d_1G
                slope_2 = d_0G / d_2G
                Dist_slope_1.append(np.nanmedian(slope_1))
                Dist_slope_2.append(np.nanmedian(slope_2))
    ...
    plt.plot(selected_flux_list_1,Dist_slope_1,'o',color=color_list[q],label='Channel {} 0G/1G'.format(q+1))
    #plt.plot(selected_flux_list_1,np.median(Dist_slope_2,axis=1),'<',color=color_list[q],label='Channel {} 0G/2G'.format(q+1))
    plt.plot(selected_flux_list_1,Gain_change_expo,'<',color=color_list[q],label='Channel {} Exponentail'.format(q+1))
...
plt.title('Comparison of gain by exponential fit and spline fit',fontsize=18)
plt.savefig('Splive_fit_slope_n_exp_fit_vs_count.png')
