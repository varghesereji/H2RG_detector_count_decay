...
def Fitting_1(channel,path,name):
...
def Fitting_2(channel,path,name):
    ...
    pvs,qvs,counts,er_p,er_q = Fitting_1(channel,path,name)
    ...
    for split in range(0,slices,1):
        ...
        interval = 250
        flag = 0
        fiber_mask = np.load('/home/varghese/Desktop/DP1/Masks/Needed_fiber_mask.npy')[split*Hor_pixels:(split+1)*Hor_pixels,512*channel:512*(channel+1)]
        ...
        for flux_bin in range(500,60000,interval):
            ...
            time1=time1/time1[-1]
#            print(delta_change_cube)
            if not np.isnan(avg_count) and delta_change_cube[-1]<0:
                P_ini=0.01
                flag+=1
                Sigma = Sigma1*100/(flux_bin*np.sqrt(np.sum(pix_mask)))
...
#                    print(channel,split,flux_bin,P)
                    '''
                    ...
                    pass
            #else:
             #   print('last element of dc is larger than 0')
             #   plt.figure()
             #   plt.plot(delta_change_cube)
             #   plt.savefig('DS5_exceptional_curves_{0}_{1}.png'.format(channel,split))
...
#        print(channel,split,pvs)
    ...
    plt.xlabel('counts')
    plt.ylabel('p')
    plt.title('p vs counts from exponential,DS5-19/5 {0} Horizontal slices channel {1}'.format(slices,channel))
    #plt.show(block=False)
    plt.savefig('DS5_p_from_exp_Horizontal_{0}_Ch_{1}.png'.format(slices,channel))#.format(channel))
...
plt.title('p vs counts from exponential,DS5-19/5 {0} Horizontal slices channel {1}'.format(slices,channel))
#plt.show(block=False)
plt.savefig('DS5_p_from_exp_Horizontal_{0}.png'.format(slices))#.format(channel))
