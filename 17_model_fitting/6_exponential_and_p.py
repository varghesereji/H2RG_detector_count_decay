...
#rows = []
        ...
        if not np.isnan(avg_count) and delta_change_cube[-1]<0:
            ...
            try:
                ...
                plt.plot(time1,exponential(time1,avs[flag],bvs[flag]),'--',label='exponential from a and b')   # Polynomial with a and b
                ...
                plt.plot(time1,polynomial(time1,pvs[flag],qvs[flag]),'--',label='polynomial with p={0},q={0}'.format(pvs[flag],qvs[flag]))   # Polynomial with p and q
                plt.plot(time1,polynomial(time1,pvs[flag],objective(pvs[flag],l,m,w)),'--',label='polynomial with p only'.format(pvs[flag])) # Polynomial with p only
                plt.xlabel('time')
                plt.legend()
                plt.ylabel('count')
