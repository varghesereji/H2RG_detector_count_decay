...
def objective1(x,t,C,S):
    Objective = np.append((x[0]*(np.exp(-x[1]*t)-1)-C)/S,np.sqrt(np.abs(x[0])))
    Objective = np.append(Objective,np.sqrt(np.abs(x[1])))
    ...
    for flux_bin in range(500,60000,interval):
        ...
        if not np.isnan(avg_count) and delta_change_cube[-1]<0:
            ...
            try:
                ...
                fitting_data_variables= least_squares(objective1,x0=np.array([1.0,0.5]),args=(time1,delta_change_cube,Sigma))
                ...
                er_b.append(db)
                counts.append(avg_count)
                print('ch={0},S_a = {1}, S_b = {2},a={3},b={4}'.format(channel,da,db,a,b),'lamb=',1)
            except Exception as e:
                print(e)
